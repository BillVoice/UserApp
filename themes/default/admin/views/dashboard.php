...
<?php if (($Owner || $Admin) && $chatData) {
    ...
    <div class="box" style="margin-bottom: 15px;">
        <div class="box-header">
            <h2><i class="fa-fw fa fa-bar-chart-o"></i><?= lang('overview_chart'); ?></h2>
        </div>
        <div class="box-content">
            <div class="row">
                <div class="col-md-12">
    ...
    <script type="text/javascript">
        ...
        $('#ov-chart').highcharts({
            ...
            tooltip: {
                ...
                formatter: function() {
                    if (this.key) {
                        return '<div dir="<?= $Settings->user_rtl ? 'rtl' : 'ltr'; ?>" class="tooltip-inner hc-tip" style="margin-bottom:0;">' +
                            this.key + '<br><strong>' + currencyFormat(this.y) + '</strong> (' +
                            formatNumber(this.percentage) + '%)';
                    } else {
                        var s =
                            '<div dir="<?= $Settings->user_rtl ? 'rtl' : 'ltr'; ?>" class="well well-sm hc-tip" style="margin-bottom:0;"><h2 style="margin-top:0;">' +
                            this.x +
                            '</h2><table class="table table-striped"  style="margin-bottom:0;">';
                        $.each(this.points, function() {
                            s += '<tr><td style="color:{series.color};padding:0;text-align:<?= $Settings->user_rtl ? 'right' : 'left'; ?>;">' +
                                this.series.name +
                                ': </td><td style="color:{series.color};padding:0;text-align:right;"> <b>' +
                ...
                },
                {
                    ...
                    data: [
                        ...
                        ['<?= lang('stock_value_by_price'); ?>',
                            <?php echo $stock->stock_by_price; ?>
                        ],
                        ['<?= lang('stock_value_by_cost'); ?>',
                            <?php echo $stock->stock_by_cost; ?>
                        ],
                    ],
                    center: [80, 42],
                    size: 80,
